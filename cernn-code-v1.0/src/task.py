        ...
        for i in range(self.batch_size):
            if loc_type == "fix_in":
                ...
                try:
                    ...
                    self.stim_strength = 100 * np.full((len(locs), 4), 100.0, dtype=np.float32)
                ...
                self.y[ons[i] : offs[i], i, 1:] += self.add_y_loc(
                    locs[i]
                )  # *strengths[i] #output shouldn't be modulated by strength 20220125
...
def delaygo_(config, mode, anti_response, stim_mod, **kwargs):
    """
...
def delaygo_s1(config, mode, **kwargs):
    return delaygo_(config, mode, False, stim_mod=1, **kwargs)


def delaygo_v1(config, mode, **kwargs):
    return delaygo_(config, mode, False, stim_mod=2, **kwargs)
...
def reactgo_(config, mode, anti_response, stim_mod, **kwargs):
    """
...
def reactgo_s1(config, mode, **kwargs):
    return reactgo_(config, mode, False, stim_mod=1, **kwargs)


def reactgo_v1(config, mode, **kwargs):
    return reactgo_(config, mode, False, stim_mod=2, **kwargs)


def reactanti_s1(config, mode, **kwargs):
    return reactgo_(config, mode, True, stim_mod=1, **kwargs)


def reactanti_v1(config, mode, **kwargs):
    return reactgo_(config, mode, True, stim_mod=2, **kwargs)


def fdgo_(config, mode, anti_response, stim_mod, **kwargs):
    """
...
def fdgo_s1(config, mode, **kwargs):
    return fdgo_(config, mode, False, stim_mod=1, **kwargs)


def fdgo_v1(config, mode, **kwargs):
    return fdgo_(config, mode, False, stim_mod=2, **kwargs)


def fdanti_s1(config, mode, **kwargs):
    return fdgo_(config, mode, True, stim_mod=1, **kwargs)


def fdanti_v1(config, mode, **kwargs):
    return fdgo_(config, mode, True, stim_mod=2, **kwargs)


def delayanti_s1(config, mode, **kwargs):
    return delaygo_(config, mode, True, stim_mod=1, **kwargs)


def delayanti_v1(config, mode, **kwargs):
    return delaygo_(config, mode, True, stim_mod=2, **kwargs)
...
rule_mapping = {
    ...
    "fdgo_s1": fdgo_s1,
    "fdgo_v1": fdgo_v1,
    "reactgo_s1": reactgo_s1,
    "reactgo_v1": reactgo_v1,
    "delaygo_s1": delaygo_s1,
    "delaygo_v1": delaygo_v1,
    "fdanti_s1": fdanti_s1,
    "fdanti_v1": fdanti_v1,
    "reactanti_s1": reactanti_s1,
    "reactanti_v1": reactanti_v1,
    "delayanti_s1": delayanti_s1,
    "delayanti_v1": delayanti_v1,
    "dm1": dm1,
    "dm2": dm2,
...
rule_name = {
    ...
    "reactanti_s1": "RT Anti S",
    "reactanti_v1": "RT Anti V",
    "delayanti_s1": "Dly Anti S",
    "delayanti_v1": "Dly Anti V",
    "fdanti_s1": "Anti S",
    "fdanti_v1": "Anti V",
    "dmsgo": "DMS",
    "dmsnogo": "DNMS",
